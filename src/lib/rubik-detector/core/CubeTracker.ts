...
import type { CubeDetection, FrameResult } from "../types";
...
export class CubeTracker {
  private balHist: number[] = [];
  private sup2Hist: number[] = [];
  private fillHist: number[] = [];
  private committedN = 0;
  private pendingN = 0;
  private pendingCount = 0;
  private lost = 0;
  private last: CubeDetection | null = null;

  update(f: FrameResult): CubeDetection {
    if (!f.hull) {
      this.lost++;
      if (this.last && this.lost <= MAX_LOST) return this.last; // brief dropout → hold
      this.reset();
    ...
    let target: number;
    if (balance > BALANCE_3 && f.cand3) target = 3;
    else if (sup2 > SUP2_2 && f.cand2) target = 2;
    else target = 1;

    // Hysteresis: require the new count to persist before committing.
    if (target === this.committedN) {
      this.pendingN = target; this.pendingCount = 0;
    } else if (target === this.pendingN) {
      if (++this.pendingCount >= COMMIT) { this.committedN = target; this.pendingCount = 0; }
    } else {
      this.pendingN = target; this.pendingCount = 1;
    }
    if (this.committedN === 0) this.committedN = target; // first lock is immediate

    const n = this.committedN;
    let faces = f.cand1, center = null as CubeDetection["center"];
    if (n === 3 && f.cand3) { faces = f.cand3.faces; center = f.cand3.center; }
    else if (n === 2 && f.cand2) { faces = f.cand2; }

    const det: CubeDetection = {
      hull: f.hull,
      faces,
      nFaces: n,
      center,
  ...
  reset(): void {
    this.balHist = []; this.sup2Hist = []; this.fillHist = [];
    this.committedN = 0; this.pendingN = 0; this.pendingCount = 0; this.lost = 0; this.last = null;
  }
}
