...
export function centroid(pts: Point2[]): Point2 {
  let x = 0, y = 0;
...
export function orderQuad(pts: Point2[]): [Point2, Point2, Point2, Point2] {
...
export function rightAngleScore(pts: Point2[]): number {
  let dev = 0;
  for (let i = 0; i < 4; i++) {
...
export function squareScore(pts: Point2[]): number {
  const sides = [
    dist(pts[0], pts[1]),
    dist(pts[1], pts[2]),
    dist(pts[2], pts[3]),
...
export function centerScore(pts: Point2[], W: number, H: number): number {
  const c = centroid(pts);
...
export function variance(pts: Point2[]): number {
  const c = centroid(pts);
  let v = 0;
  for (const p of pts) v += (p.x - c.x) ** 2 + (p.y - c.y) ** 2;
  return v / pts.length;
}

export function pointInPoly(pt: Point2, poly: Point2[]): boolean {
  let inside = false;
...
function perpDist(p: Point2, a: Point2, b: Point2): number {
  const dx = b.x - a.x, dy = b.y - a.y;
  const len = Math.hypot(dx, dy) || 1;
  return Math.abs((p.x - a.x) * dy - (p.y - a.y) * dx) / len;
}

function dpOpen(pts: Point2[], eps: number): Point2[] {
...
export function dpClosed(hull: Point2[], eps: number): Point2[] {
  const n = hull.length;
  ...
  const arc1: Point2[] = [];
  for (let i = a; i !== b; i = (i + 1) % n) arc1.push(hull[i]);
  arc1.push(hull[b]);
  const arc2: Point2[] = [];
  for (let i = b; i !== a; i = (i + 1) % n) arc2.push(hull[i]);
  arc2.push(hull[a]);
  return dpOpen(arc1, eps).slice(0, -1).concat(dpOpen(arc2, eps).slice(0, -1));
}

// Keep the k sharpest-turn corners (in original order).
export function topKCorners(poly: Point2[], k: number): Point2[] {
  const n = poly.length;
  if (n <= k) return poly;
...
export function parallelogramScore(q: Point2[]): number {
  const [a, b, c, d] = q;
  ...
  const dir = (p: Point2, q2: Point2) => { const dx = q2.x - p.x, dy = q2.y - p.y, l = Math.hypot(dx, dy) || 1; return [dx / l, dy / l]; };
...
export function minAreaRect(hull: Point2[]): { corners: [Point2, Point2, Point2, Point2]; area: number } | null {
  if (hull.length < 3) return null;
  let best: { area: number; corners: Point2[] } | null = null;
  const n = hull.length;
  for (let i = 0; i < n; i++) {
    ...
    if (!best || area < best.area) {
      const corner = (uu: number, vv: number): Point2 => ({ x: ux * uu + vx * vv, y: uy * uu + vy * vv });
      best = {
        area,
