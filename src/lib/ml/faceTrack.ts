...
export class FaceTracker {
  ...
  update(face: DetectedFace | null): Slot[] {
    ...
    // (re)acquire when there's no track or the face jumped far (new position)
    if (!this.slots || d2(cx, cy, this.centroid().x, this.centroid().y) > pitch * 2.2) {
      this.slots = cells.map((c) => ({ gx: c.gx, gy: c.gy, cx: c.center.x, cy: c.center.y, corners: c.corners.map((p) => ({ ...p })), hist: [c.colour], miss: 0, detected: c.detected }));
      return this.slots;
    }
