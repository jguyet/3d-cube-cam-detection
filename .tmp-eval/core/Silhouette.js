...
class Silhouette {
    ...
    detect(img, sThresh = 0.5, vThresh = 0.25) {
        ...
        for (let i = 0; i < cubeSeed.length; i++)
            cubeSeed[i] = colorMask[i] || whiteSeed[i] ? 1 : 0;
        const support = this.dilate(cubeSeed, w, h, 6);
        const whiteExpanded = this.dilate(whiteCandidate, w, h, 1);
        ...
        const filteredWhite = this.filterWhiteComponents(whiteCandidate, cubeSeed, support, darkSupport, skinExpanded, movingNow, movingRecent, w, h);
        const whiteExpandedFiltered = this.dilate(filteredWhite, w, h, 1);
        let mask = new Uint8Array(colorMask);
        for (let y = 3; y < h - 3; y++)
            for (let x = 3; x < w - 3; x++) {
                const i = y * w + x;
                if (!whiteExpandedFiltered[i] || !support[i] || skinExpanded[i])
                    continue;
        ...
        if (!blob || blob.length < w * h * 0.015)
            return { hull: null, fillFrac: 0 };
        const hull = (0, geometry_1.convexHull)(blob);
        return { hull, fillFrac: (0, geometry_1.polygonArea)(hull) / (w * h) };
    }
    dilate(m, w, h, iters) {
            ...
            for (let y = 1; y < h - 1; y++)
                for (let x = 1; x < w - 1; x++) {
                    ...
                    if (m[i] || m[i - 1] || m[i + 1] || m[i - w] || m[i + w] ||
                        m[i - w - 1] || m[i - w + 1] || m[i + w - 1] || m[i + w + 1])
                        o[i] = 1;
                }
            m = o;
        }
    ...
    filterWhiteComponents(white, cubeSeed, support, darkSupport, skin, movingNow, movingRecent, w, h) {
        ...
        for (let s = 0; s < w * h; s++) {
            ...
            if (!plausibleSize || !anchored || !skinSafe || !(temporal || staticStructured))
                continue;
    ...
    ensureTemporalBuffers(w, h) {
        if (this.bgW === w && this.bgH === h && this.bgY && this.bgCb && this.bgCr && this.motionHold)
            return;
        this.bgW = w;
        this.bgH = h;
    ...
    computeMovingMask(luma, cbMap, crMap, w, h) {
    ...
    updateBackground(luma, cbMap, crMap, mask, movingNow, w, h) {
        const bgY = this.bgY;
        const bgCb = this.bgCb;
        const bgCr = this.bgCr;
        const protectedMask = this.dilate(mask, w, h, 4);
        for (let i = 0; i < bgY.length; i++) {
            if (protectedMask[i] || movingNow[i])
                continue;
